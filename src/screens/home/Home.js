...
const Home = () => {
  ...
  const filterHandler = () => {
    ...
    displayedMovies = displayedMovies.filter((movie) => {
      ...
      else if (isNaN(endDate)) return startDate <= releaseDate;
      else if (isNaN(startDate)) return releaseDate <= endDate;
      else return startDate <= releaseDate && releaseDate <= endDate;
    });
                ...
                >
                  {genres.map((genre) => (
                    <MenuItem key={genre.id} value={genre.genre}>
                      <Checkbox
                        color="primary"
                        checked={genreChoice.indexOf(genre.genre) > -1}
                      />
